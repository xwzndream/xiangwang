...
import {
  AppstoreOutlined, BellOutlined, CustomerServiceOutlined, LogoutOutlined, PlusOutlined,
...
type Project = { id:string; customer_id:string; name:string; project_type:string; status:string; delivered_at?:string; service_expires_at?:string; offline_at?:string };
type Dashboard = { customers:Customer[]; leads:Lead[]; projects:Project[] };
...
function AdminApp() {
  ...
  const [data,setData] = useState<Dashboard>({customers:[],leads:[],projects:[]});
  ...
  const load = async () => {
    ...
    try {
      ...
      setData({customers:result.customers||[],leads:result.leads||[],projects:result.projects||[]});
  ...
  const projectAction = async (project:Project, action:string) => {
    const titles:Record<string,string> = {offline:"确认记录网站已下架？",deliver:"确认项目已交付？",complete:"确认项目已完成？"};
    const success:Record<string,string> = {offline:"已记录网站下架",deliver:"项目已标记为交付",complete:"项目已标记为完成"};
  ...
  const projectColumns = [{title:"项目",dataIndex:"name"},{title:"类型",dataIndex:"project_type",render:(v:string)=>v==="landing_page"?"静态官网 / 个人落地页":"其他套餐项目"},{title:"状态",dataIndex:"status",render:(v:string)=><Tag color={v==="delivered"||v==="completed"||v==="active"?"green":v==="offline"?"default":"orange"}>{statusLabels[v]||v}</Tag>},{title:"交付日期",dataIndex:"delivered_at",render:(v:string)=>v?dayjs(v).format("YYYY-MM-DD"):"尚未交付"},{title:"网站免费管理到期",render:(_:unknown,p:Project)=>p.project_type==="landing_page"?(p.service_expires_at?dayjs(p.service_expires_at).format("YYYY-MM-DD"):"交付后计算"):"不适用"},{title:"交付管理",render:(_:unknown,p:Project)=><Space>{!p.delivered_at&&<Button size="small" type="primary" onClick={()=>projectAction(p,"deliver")}>标记已交付</Button>}{p.status!=="completed"&&<Button size="small" onClick={()=>projectAction(p,"complete")}>标记已完成</Button>}{p.project_type==="landing_page"&&p.status!=="offline"&&<Button size="small" danger onClick={()=>projectAction(p,"offline")}>记录已下架</Button>}</Space>}];
