...
export default async function handler(req) {
  ...
  try {
    ...
    if (action === "dashboard" && req.method === "GET") {
      const [customers, leads, projects, connections] = await Promise.all([
        rows(client, "customers"), rows(client, "leads"), rows(client, "projects"), rows(client, "provider_connections"),
      ]);
      return json({ customers, leads, projects, connections });
    }
